function viewRec(opts) {
    ...
    var timeCache;
    var index = 0;
    var config = $.extend({interval: 500, mark_classname: 'vr-mark',
    pointer_idname: 'vr-pointer', target: window}, opts);
    ...
    var _updateData = function() {
        ...
        cache.id = index;
        timeCache = newTime;
        data.push([cache.pointer.x, cache.pointer.y, cache.scroll.x, 
            cache.scroll.y, cache.btn, cache.time, cache.id]);
        index++;
        cache.btn = 0;
    ...
    fn.play=function() {
        if (!$('#' + config.pointer_idname).length) {
            $('body').after('<div id="' + config.pointer_idname + 
        ...
        if (!play) {
            if (typeof data[i] !== 'undefined') {
                play = setInterval(function() {
                    if (typeof data[i] !== 'undefined') {
                        jquery.trigger('play');
                        $('#'+config.pointer_idname).animate({top:data[i][1],
                        left:data[i][0]}, data[i][5]);
                        $(window).scrollTop(data[i][3]);
                        $(window).scrollLeft(data[i][2]);
                        
                    if (data[i][4] != 0) {
                        var btn = data[i][4] + ' Click';
                        $('body').after('<div class="' + config.mark_classname + 
                        ' ' + btn + '" style="position: absolute; top:' + data[i][1]+
                        'px;left:' + data[i][0] + 'px;">' + btn + '</div>')
                    };
                    i++;
                    console.log(data[i])
                    }
